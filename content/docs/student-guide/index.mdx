...
<Section title="1-1. Unreal Engine 5.7 を起動する" goal="Unreal Engine 5.7 が起動してランチャー画面が表示された状態">
...
**Branch** は「**条件**が True なら **True** 側の処理へ、False なら **False** 側の処理へ進む」ためのノードです。今回は「**Score が 5 以上か？**」を条件にして、True のときだけ `CLEAR!` を表示します。

条件の比較には「**>=**（以上）」ノードを使い、「Score の値」と「5」を比べて True / False を返してもらいます。
