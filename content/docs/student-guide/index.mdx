...
<Action img="./img/convert-to-blueprint-window-process.png">
  グレーの破線で囲まれた ①②④ は画像の通りになっているか見比べて、違っていれば画像と同じ状態にします（最初から画像の通りになっているはずです）。オレンジの実線で囲まれた ③ のブループリント名に「**BP_Item**」と入力して、⑤「**選択**」をクリックします
</Action>
...
<Action>
  手前に自動追加された**「緑から紫」の変換ノード**（`Set Score` の出力と `Print String` の間にある小さなノード）も同様に選択して **Delete キー**で削除します
</Action>
