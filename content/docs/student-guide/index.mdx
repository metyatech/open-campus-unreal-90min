...
<Verify img="./img/unrealeditor.png">読み込みが完了してメイン画面が開けば起動成功です（数分かかることがあります）</Verify>
...
<Verify img="./img/cube-positioned-low-enough-to-touch.png">キューブがキャラクターに触れられる高さ（画像くらい）に置かれていれば配置完了です</Verify>
...
<Verify img="./img/after-convert-to-blueprint.png">コンテンツブラウザに `BP_Item` が作成されて編集画面が自動で開き、ビューポート上のキューブも BP_Item に置き換わっていれば変換成功です</Verify>
...
<Verify img="./img/BP_Item-print-string-from-actorbeginoverlap.png">イベントグラフがこの形になっていれば成功です</Verify>
...
<Verify img="./img/compile-success.png">コンパイルアイコンが**緑のチェック**に変わっています</Verify>

<Action>
  メインエディタに戻ってプレイ（▶）して、キャラクターをアイテムに触れさせます
</Action>

<Verify img="./img/print-string-Hello-in-viewport.png">画面左上に水色で「Hello」と表示されれば成功です</Verify>
...
<Verify img="./img/Event%20ActorBeginOverlap-Cast%20To%20BP_ThirdPersonCharacter.png">`Event ActorBeginOverlap` と `Cast To BP_ThirdPersonCharacter` が画像の通りにつながっていれば配置完了です</Verify>
...
<Verify img="./img/increment-score-overview.png">コンパイルアイコンが**緑のチェック**になり、Blueprint 全体がこの形になっていれば成功です</Verify>
...
<Action img="./img/auto-conversion-node-before-delete.png">
...
<Action img="./img/is-score-greater-than-or-equal-to-5.png">
...
<Verify img="./img/print-string-clear-result.png">5 個目を取った瞬間に、画面左上に「CLEAR!」と表示されれば完成です</Verify>
