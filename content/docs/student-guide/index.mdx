...
<Step goal="キャラクターを自分で動かせるようになります">
...
三人称テンプレートを使うことで、移動・ジャンプ・カメラが最初から動く状態で始められます。
...
<Step goal="ステージの上に取れるアイテムが並んだ状態になります">
...
<Procedure why="まず 1 つ置いて、位置調整の操作を確認します">
  ...
  <Verify>キューブがステージ上の指定した位置に置かれます。</Verify>
</Procedure>

:::caution[矢印（移動ギズモ）が表示されないとき]
キューブを選択して **W キー** を押すか、![平行移動に切り替えボタン](./img/translate-button.png) をクリックして移動モードに切り替えてください。
:::

### 2-2. 動作確認

<Procedure why="次のステップで仕組みを追加する前に、今の状態を確認します">
  <Action>
...
<Procedure why="キューブを Blueprint クラスにして、動作を設定できるようにします">
...
<Procedure why="同じ設定を持つアイテムを 5 個ステージに置きます">
...
<Step goal="アイテムに触れるたびにアイテムが消えてスコアが増えるようになります">
...
<Procedure why="Score 変数をプレイヤーキャラクターに追加します">
...
<Procedure why="オーバーラップに変えて、触れたことを検知できるようにします">
  ...
  <Verify>プレイヤーがアイテムをすり抜けて通れるようになります。</Verify>
</Procedure>

### 3-3. Overlap イベントで動作確認する

アイテムに触れたとき、画面に「Hello」という文字が表示されるようにします。まずこれで「**触れたことを検知する**」仕組みの作り方を学びます。

<Procedure why="Blueprint エディタの全機能が使える画面に切り替えます">
...
<Procedure why="「Hello」が表示されれば触れた検知が正しく動いています">
...
「コンパイル」とは、**作ったノードの設定をゲームが実際に動かせる形に変換する作業**です。アイコンが緑のチェック → 成功、赤の × → エラーあり。ノードを組み終わったら一度押してアイコンを確認する習慣をつけておくと、問題を早く発見できます。
...
<Procedure why="ゲームをプレイして Hello が表示されることを確認します">
  <Action>
...
<Procedure why="確認用ノードを削除して次の手順をきれいな状態で始めます">
  <Action>
...
<Procedure why="触れたオブジェクトが本当にプレイヤーかどうかを確認するノードを追加します">
...
<Procedure why="現在のスコアを取得して 1 を加算します">
...
<Procedure why="計算した値で Score 変数を更新します">
...
<Procedure why="スコアが正しく増えているか確認してから削除します">
...
<Procedure why="確認用ノードを削除して仕上げます">
  <Action>
    `Print String` ノードをクリックして選択し、**Delete キー**を押して削除します
  </Action>
  <Action>
...
<Procedure why="スコアが増えた後にアイテム自体も消します">
...
<Procedure why="アイテムが消えることを確認して Step 3 を完了します">
  <Action>
    メインエディタに戻ってプレイ（▶）して、アイテムに触れます
  </Action>
...
<Step goal="アイテムを 5 個すべて集めた瞬間に画面に「CLEAR!」と表示されるゲームが完成します">
...
<Procedure why="スコアが 5 以上のときに CLEAR! へ進む分岐を作ります">
...
<Procedure why="スコアが 5 以上のときに CLEAR! を表示します">
...
<Procedure why="クリア条件が正しく機能することを確認します">
  <Action>
