...
<Action img="./img/newproject.png">
  「**新規プロジェクト**」をクリックすると「新規プロジェクト」ウィンドウが開きます
</Action>
...
<Section title="1-3. プレイして動かしてみる" goal="ゲームをテストプレイする方法が分かります">
...
<Action img="./img/convert-to-blueprint-window-process.png">
  **グレーの破線**で囲まれた **①②④** は画像の通りになっているか見比べて、違っていれば画像と同じ状態にします（最初から画像の通りになっているはずです）。**オレンジの実線**で囲まれた **③** のブループリント名に「**BP_Item**」と入力して、**⑤**「**選択**」をクリックします
</Action>

<Action img="./img/after-convert-to-blueprint.png">
  「**選択**」をクリックすると、コンテンツブラウザに **BP_Item** が作成され、編集画面が自動で開きます
</Action>

<Action img="./img/cube-converted-to-BP_Item.png">
  ビューポート上のキューブも **BP_Item** として置き換わります
</Action>

<Verify>コンテンツブラウザに `BP_Item` が表示され、ビューポート上のキューブもそれと同じオブジェクトに切り替わっていれば変換成功です。</Verify>
...
<Action img="./img/add-Score-variable.png">
  「マイブループリント」パネルの「変数」の右の ①「**＋**」から、② 名前「**Score**」・③ 型「**Integer**」の変数を追加します
</Action>
...
<Action>
  コンテンツブラウザで「**BP_Item**」をダブルクリックして「**BP_Item**」の編集画面を開きます
</Action>
...
<Verify img="./img/increment-score-overview.png">Blueprint 全体がこの形になっていれば成功です</Verify>
