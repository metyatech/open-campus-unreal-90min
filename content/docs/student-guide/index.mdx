...
<Section title="1-1. Unreal Engine 5.7 を起動する" goal="Unreal Engine 5.7 が起動しています">
...
<Section title="1-2. テンプレートを選ぶ" goal="移動・ジャンプ・カメラが最初から動く三人称テンプレートでプロジェクトが開かれています">
...
<Section title="1-3. プレイして動かしてみる" goal="WASD / Space でキャラクターを操作できます">
...
<Section title="2-1. キューブをステージに置く" goal="歩いて触れる位置にキューブが 1 つ置かれています">
...
<Section title="2-2. 動作確認" goal="今のキューブはただの 3D 物体で、触れても何も起きないことを確認できます">
...
<Section title="2-3. キューブを Blueprint クラスに変換する" goal="キューブが BP_Item という Blueprint クラスになっています">
...
<Section title="2-4. BP_Item をレベルに 5 つ配置する" goal="ステージ上に BP_Item が 5 個配置されています">
...
<Section title="3-1. プレイヤーキャラクターに Score 変数を追加する" goal="BP_ThirdPersonCharacter に Integer 型の Score 変数が追加されています">
...
<Section title="3-2. BP_Item のコリジョン設定を変える" goal="プレイヤーが BP_Item をすり抜けて通れます">
...
<Section title="3-3. Overlap イベントで動作確認する" goal="アイテムに触れると画面に「Hello」が表示されます">
...
<Section title="3-4. Blueprint ロジックを組む" goal="触れたらスコアが +1 されるロジックがイベントグラフ上に組まれています">
...
<Section title="(a) Cast ノードを追加する" goal="触れたオブジェクトがプレイヤーかを判定するノードがつながっています">
...
<Section title="(b) 現在のスコアを取得して 1 を加算する" goal="Score + 1 を計算する Add ノードがつながっています">
...
<Section title="(c) 計算した値で Score 変数を更新する" goal="加算後の値が Set Score ノードに入力されるようつながっています">
...
<Section title="(d) 動作確認用の Print String をつないで増分を確認する" goal="プレイ中にスコアの増分が画面で確認できます">
...
<Section title="(e) 動作確認用ノードを片付ける" goal="動作確認用の Print String と変換ノードが削除されています">
...
<Section title="3-5. アイテムを消す（Destroy Actor）" goal="スコア加算後にアイテム自体も消えるロジックが完成しています">
...
<Section title="3-6. テストプレイで確認する" goal="アイテムに触れるとスコアが増えてアイテムが消えることを確認できます">
...
<Section title="4-1. BP_Item にクリア判定を追加する" goal="スコアが 5 以上のときに CLEAR! を表示する分岐がイベントグラフに組まれています">
...
<Section title="4-2. テストプレイで確認する" goal="5 個目のアイテムを取った瞬間に「CLEAR!」が表示されることを確認できます">
